...
class MesLocatairesPage(QWidget):
    ...
    def suppr_clicked(self):
        ...
        if item_ref is None:
            return
        locataire_id = item_ref.data(1)
        locataire = self.recette_service.get_recette_by_id(locataire_id)
        ...
        try:
            if confirm_and_delete(self.patrimoine_service, scenario, locataire, self):
                self.load()
    ...
    def load(self):
        ...
        if scenario:
            ...
            for i,locataire in enumerate(locataires):
                if locataire.date_in <= date_seuil:
                    bien_loué = self.invest_service.get_by_id(locataire.id_source)
                    print(bien_loué)
                    try:
        ...
        self.navigator.hold_page(Page.MES_LOCATAIRES)