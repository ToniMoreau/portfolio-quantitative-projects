...
class AjouterCreditPage(QWidget):
    ...
    def __init__(self, appContext : AppContext, session : Session):
        ...
        self.debut_month_input.editingFinished.connect(self.update_capa_emprunt)
        self.debut_month_input.setPlaceholderText("Mois")
        self.debut_month_input.setFixedWidth(60)
        self.debut_month_input.setValidator(QIntValidator(0,12))
        
        self.debut_year_input = QLineEdit()
        self.debut_year_input.setFixedWidth(60)
        self.debut_year_input.setValidator(QIntValidator(1980,2200))
        self.debut_year_input.setPlaceholderText("Année")
        self.debut_year_input.editingFinished.connect(self.update_capa_emprunt)
        ...
        #actions boutons
        self.annuler_btn.clicked.connect(lambda : self.navigator.go_to(Page.CREDITS))
    ...
    def enregistrer_clicked(self):
        ...
        month_debut_credit = self.debut_month_input.text().strip()
        year_debut_credit = self.debut_year_input.text().strip()
        ...
        if credit is None:
            ...
            date_in = None if not(month_debut_credit and year_debut_credit) else date(int(year_debut_credit), int(month_debut_credit), 1)
            ...
            date_in = credit.debut if not(month_debut_credit and year_debut_credit) else date(int(year_debut_credit), int(month_debut_credit), 1)
