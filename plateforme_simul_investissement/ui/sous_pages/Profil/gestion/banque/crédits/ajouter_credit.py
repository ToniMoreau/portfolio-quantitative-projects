...
class AjouterCreditPage(QWidget):
    ...
    def __init__(self, appContext : AppContext, session : Session):
        ...
        info_layout.addWidget(self.etendu_wgt)
    ...
    def test_limite_montant_input(self, montant : str):
        ...
        try :
            ...
            bas = 100
            capa_annuelle = round(self.capa_mensu,2) * 360
        ...
        if type_actif == self.mensualite_duree:            
            ...
            if duree and mensu:
                if self.test_limite_duree_input(duree) and self.test_limite_mensu_input(mensu):
                    mensu = float(mensu)
                    duree = int(duree)
                    montant = round(self.credit_service.montant_from_mensu_duree(duree_mois=duree, mensualite=mensu, taux = banque.taux_credit_pct, duree_differe_mois=ddiff_mois),2)
    ...
    def enregistrer_clicked(self):
        ...
        print(self.banque_choix.currentData())
        print("banque", banque)
        ...
        if credit is None:
            id = None
            banque_id = None if banque is None else banque.id
            compte_choix = None if compte_choix is None else compte_choix
            montant = None if montant is None else float(montant)
            duree_credit = None if duree_credit is None else duree_credit
            mensu = None if mensu is None else float(mensu)
            duree_differee = 0 if duree_differee is None else duree_differee
            mois_debut_credit = None if mois_debut_credit is None else mois_debut_credit
            taux_interet = None if banque is None else banque.taux_credit_pct
            
        else:
            id = credit.id
            banque_id = credit.id_banque if banque is None else banque.id
            compte_choix = credit.id_compte if compte_choix is None else compte_choix
            montant = credit.montant if montant is None else float(montant)
            duree_credit = credit.durée_crédit_mois if duree_credit is None else duree_credit
            mensu = credit.mensualite_constante if mensu is None else float(mensu)
            duree_differee = credit.duree_diff_mois if duree_differee is None else duree_differee
            mois_debut_credit = credit.debut if mois_debut_credit is None else mois_debut_credit
        ...
        print(duree_credit)
        print(mensu)
        print(duree_differee)   
        print(mois_debut_credit)
