...
class CreditService:
    ...
    def capacite_emprunt(self, id_scenario, date_emprunt):
        # revenus (salaires, locatifs - 70%)
        #crédits en cours
        #35% d'endettement
        ENDETTEMENT_TAUX = 0.35
        ...
        revenu_mensuels = self.recette_repo.get_by_({"ID SCENARIO" : id_scenario, "NATURE" : "Revenus", "FREQUENCE" : "Mensuel"})
        revenus_locatifs = self.recette_repo.get_by_({"ID SCENARIO" : id_scenario, "NATURE" : "Revenus locatifs", "FREQUENCE" : "Mensuel"}) 
        credits = self.depense_repo.get_by_({"ID SCENARIO" : id_scenario, "NATURE" : "Crédit", "FREQUENCE" : "Mensuel"})
        total_revenus = 0
        for revenu_mensuel in revenu_mensuels:
            date_in =revenu_mensuel.date_in 
            date_out = revenu_mensuel.date_out
            retrospective = date_emprunt -6
            if date_in <= date_emprunt <=date_out:
                etendu = date_emprunt - max(retrospective,date_in)
            elif date_emprunt < date_in:
                etendu = 0
            else:
                etendu = max(retrospective,date_out) - max(date_in,retrospective)
            revenu = revenu_mensuel.montant * etendu
            total_revenus += revenu
            
        for revenus_locatif in revenus_locatifs:
            date_in =revenus_locatif.date_in 
            date_out = revenus_locatif.date_out
            retrospective = date_emprunt -6
            if date_in <= date_emprunt <=date_out:
                etendu = date_emprunt - max(retrospective,date_in)
            elif date_emprunt < date_in:
                ...
                etendu = max(retrospective,date_out) - max(date_in,retrospective)
            revenu = revenus_locatif.montant * etendu
            total_revenus += revenu *0.7
            
        total_revenus /= MOIS_INCLU
            
        total_revenus*=ENDETTEMENT_TAUX
