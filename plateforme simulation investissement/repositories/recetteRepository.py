...
class RecetteRepository:
    ...
    def _rows_to_recette(self, rows: pd.DataFrame) -> list[Recette]:
        ...
        for _, row in rows.iterrows():
            recette = Recette(
                ...
                id_transaction= 0 if pd.isna(row.get("ID TRANSACTION")) else int(row["ID TRANSACTION"]),
    ...
    # --------- writes ---------
    def create(self, recette: dict[str, Any], is_transaction : bool | None = None) -> dict[str, Any]:
        """
