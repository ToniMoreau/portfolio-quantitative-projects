...
class CompteBancaireRepository:
    ...
    def _rows_to_compteBancaire(self, rows: pd.DataFrame) -> list[CompteBancaire]:
        ...
        for _, row in rows.iterrows():
            cb = CompteBancaire(
                ...
                solde_initial=float(row["SOLDE INITIAL"]),
                taux_annuel= 0 if pd.isna(row.get("RENDEMENT (%/AN)")) else float(row["RENDEMENT (%/AN)"]),
                type=s(row.get("TYPE"))
            )
            cbs.append(cb)
    ...
    def get_by_(self, dict_str_int : dict[str, int]) -> list[CompteBancaire]:
        df = self._load_df()
